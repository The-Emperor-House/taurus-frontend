...
export default function Navbar() {
  ...
  const renderAuthSection = (isMobile = false) => {
    ...
    return isMobile ? (
      <Link
        href="/auth/login"
        className={`block text-xl py-3 px-4 rounded-lg text-white text-center ${hoverEffect.text}`}
        onClick={() => setIsOpen(false)}
      >
        Login
      </Link>
    ) : (
      <Link href="/auth/login" className={`text-xl font-light ${hoverEffect.text}`}>
        Login
      </Link>
    );
  };
  ...
  return (
    <nav className="bg-[#404040] text-white p-4 font-poppins fixed w-full z-50 shadow-md">
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden bg-[#404040] overflow-hidden transition-all duration-300 ${
